...
Use this skill when an instructor needs a slide deck — either for facilitating a case discussion in class or for a general lecture. This skill works across all McCombs departments — Strategy, Marketing, Finance, Operations, MIS, and Accounting. See `../shared/discipline-guide.md` for department-specific guidance on frameworks and discussion questions. The skill produces a polished .pptx file with McCombs branding (Burnt Orange, Arial/Arial Black, widescreen 16:9 format, logo placement).
...
```bash
node /path/to/skills/slide-generator/scripts/generate_slides.js <input.json> <output.pptx>
```
