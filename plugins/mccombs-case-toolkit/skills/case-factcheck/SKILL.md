...
2. **Apply corrections** to `case.md` using the Edit tool. Apply only the corrections the instructor approved.
3. **Regenerate the .docx** using the Case Generator's bundled docx script (see `../case-generator/scripts/generate_case_docx.js`). Build a JSON payload from the corrected case.md following the same format the Case Generator uses, then run the script to produce the branded .docx.
...
- `../case-generator/references/case-structure.md`: case writing guidelines (for understanding case structure)
- `../shared/discipline-guide.md`: department-specific guidance
