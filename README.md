...
1. CI automatically validates the pull request:
   ...
   - **Cross-platform classification** — each skill is tagged `Both platforms`, `Both (see notes)`, `Needs local software` (runs command-line programs that hosted platforms don't preinstall; best in Claude Code), or `Claude only` (depends on Claude-side features like MCP connectors), based on frontmatter, bundled scripts, allowed-tools, and file references
